...
public class JdbcUser implements UserRepo {
	...
	public User findByUserName(String fullname) {	
		System.out.println("fullname " + fullname.toString().toLowerCase().trim());		
		return  template.queryForObject("select  fullname, email ,phonenumber , password from users where fullname =?  limit 1",
				this::mapRowUserDetail, 
				fullname.toString().toLowerCase().trim()); 
	}
    ...
    public ArrayList<User> getUsers() { 
		return  (ArrayList<User>) template.query("select  fullname, email ,phonenumber , password  from Users", this::mapRowUserDetail); 
